...
void printTopView(Node *root){
	map<int, int> hdToNodemap; // horizontal distance to node value
...
void printBottomView(Node *root){
	map<int, int> hdToNodemap; // horizontal distance to node map
	queue<pair<Node *, int>> q;
	q.push(make_pair(root, 0));
...
void boundaryTraversal(Node *root){
	...
	// First version written in the class
	...
	// }
	// else {
	// 	printRightBoundary(root->left); // If there is no right child, we can print the left child as the right boundary
	// }

	// Final version written in the class
	// cout << root->data << " ";
	// printLeftBoundary(root->left);
	// printLeafBoundary(root->left);
	// printLeafBoundary(root->right);
	...
	// What I think the final version should be:
	cout << root->data << " ";
	// if (root->left) printRightBoundary(root->left);
	// else printRightBoundary(root->right);
	root->left ? printRightBoundary(root->left) : printRightBoundary(root->right);
	printLeafBoundary(root->left);
	printLeafBoundary(root->right);
	// if (root->right) printRightBoundary(root->right);
	// else printRightBoundary(root->left);
	root->right ? printRightBoundary(root->right) : printRightBoundary(root->left);
}
